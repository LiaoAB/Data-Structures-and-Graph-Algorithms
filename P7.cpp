...
Tree *buildHuffmanTree(vector<pair<unsigned char, int> > frequencyTable)
{
    ...
    {
        Tree *node = new Tree();
        node->frequency = frequencyTable[i].second;
        node->character = frequencyTable[i].first;
...
string toBinary(unsigned char a)
{
    ...
    reverse(output.begin(), output.end()); //需要手刻
    return output;
...
void compressFile(char *path, char *output_path, map<unsigned char, string> &codes)
{
    ...
    Tree *root = buildHuffmanTree(convertToVector(frequencyTable));
    cout<<root<<endl;
...
{   
    ...
    string encodedOutputName;
    string decodedOutputName;
    cout << "Input file name: " << endl ;
    getline(cin, inputFileName);
    cout << "Encoded file name: " << endl ;
    getline(cin, encodedOutputName);
    decodedOutputName = "decoded" + encodedOutputName;

    char* dEncodedInput = (char*)inputFileName.c_str();
    char* dEncodedOutput = (char*)encodedOutputName.c_str();
    char *dDecodedOutput = (char*)decodedOutputName.c_str();

    if(argc==4)
    {
        dEncodedInput = argv[1];
        dEncodedOutput = argv[2];
        dDecodedOutput = argv[3];
    }
    map<unsigned char, string> codes;
    compressFile(dEncodedInput, dEncodedOutput, codes);
    decompressFile(dEncodedOutput, dDecodedOutput);
}